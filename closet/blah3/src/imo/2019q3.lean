...
-- TODO simp lemmata for the symmetric case?
...
lemma degree_of_add_edge {G : simple_graph V} (a b : V) (h : a ≠ b)
  (h_not_adj : ¬ G.adj a b) (x : V) :
  (G ⊔ bone ⟦(a, b)⟧ h).degree x =
  G.degree x + ite (x = a) 1 0 + ite (x = b) 1 0 :=
begin
  dsimp only [degree],

  -- TODO this lemma should be moved somewhere else...?
  -- TODO percolate it one layer down...???? help
  have : ∀ (G₁ G₂ : simple_graph V),
    (G₁ ⊔ G₂).neighbor_finset x = G₁.neighbor_finset x ∪ G₂.neighbor_finset x,
  { intros, ext y, simp [neighbor_finset, neighbor_set, sup_adj] },

  rw [this, finset.card_disjoint_union],
  { simp only [neighbor_finset, set.to_finset_card, card_neighbor_set_eq_degree],
    rw [add_assoc, add_right_inj],
    -- degree of bone should be calculable? this sucks
    simp only [degree, neighbor_finset, neighbor_set, bone_adj],
    -- degree_pos_iff_exists_adj,
    }
end

-- TODO a smarter strategy for this proof?
-- the neighbour set of the sup graph is the same as that of G,
-- with possibly a single edge added.
-- how do we convince lean that this is true?
lemma degree_of_add_edge {G : simple_graph V} (a b : V) (h : a ≠ b)
  (h_not_adj : ¬ G.adj a b) (x : V) :
  (G ⊔ bone ⟦(a, b)⟧ h).degree x =
  G.degree x + ite (x = a) 1 0 + ite (x = b) 1 0 :=
begin
  -- TODO new proof strategy.
  -- the two neighbour sets are disjoint.
  -- rewrite card of neighbour set union.
  -- prior art: card neighbour set compl blah blah something.

  zify,
  rw [add_assoc, ← sub_eq_iff_eq_add'],
  simp only [degree, neighbor_finset_eq_filter,
    finset.card_eq_sum_ones, finset.sum_filter],
  push_cast, rw ← finset.sum_sub_distrib, norm_num1,

  convert_to finset.univ.sum (λ y, ite (⟦(x, y)⟧ = ⟦(a, b)⟧) (1 : ℤ) 0) = _,
  { apply congr_arg, ext y,
    simp only [sup_adj, bone_adj],
    -- TODO general proofs for this graph adj of eq sym2 stuff.
    -- something like, adj maps through morphism so it respects stuff.
    -- FIXME imma call this important
    have : G.adj x y → ¬ ⟦(x, y)⟧ = ⟦(a, b)⟧,
    { intros h₁ h₂, rw sym2.eq_iff at h₂,
      rcases h₂ with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩,
      exact absurd h₁ h_not_adj,
      exact absurd h₁.symm h_not_adj },
    by_cases hp : G.adj x y;
    by_cases hq : ⟦(x, y)⟧ = ⟦(a, b)⟧;
    tauto <|> simp [hp, hq] },

  norm_cast,
  simp_rw [← finset.sum_filter, ← finset.card_eq_sum_ones, sym2.eq_iff],
  by_cases h₁ : x = a, { simp [h₁, h, finset.filter_eq'] },
  by_cases h₂ : x = b, { simp [h₂, h.symm, finset.filter_eq'], },
  simp [h₁, h₂, h],
end

lemma degree_of_delete_edge {G : simple_graph V} (a b : V)
  (h_adj : G.adj a b) (x : V) :
  ((G.delete_edges {⟦(a, b)⟧}).degree x : ℤ) =
  (G.degree x - ite (x = a) 1 0 - ite (x = b) 1 0 : ℤ) :=
begin
  suffices : G = G.delete_edges {⟦(a, b)⟧} ⊔ bone ⟦(a, b)⟧ (G.ne_of_adj h_adj),
  { rw [sub_sub, eq_sub_iff_add_eq, ← add_assoc], symmetry,
    norm_cast,
    conv_lhs { rw this },
    convert degree_of_add_edge a b _ _ x,
    simp },
  ext u v,
  simp only [sup_adj, bone_adj, delete_edges_adj, set.mem_singleton_iff],
  have : ⟦(u, v)⟧ = ⟦(a, b)⟧ → G.adj u v,
  { intro h, rw sym2.eq_iff at h,
    rcases h with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩,
    exact h_adj, exact h_adj.symm },
  tauto,
end
...
begin
  ...
  dsimp only [move.resolve],
  rw [set.insert_eq, ← delete_edges_delete_edges],
  rw degree_of_add_edge, push_cast,
  rw [degree_of_delete_edge, degree_of_delete_edge],
  { ring_nf, exact dvd_mul_right _ _ },
  { exact m.hab }, -- TODO move simp lemmas not working? trivial tactic?
  { simp [m.hac, m.bc_ne.symm] },
  { simp [m.hbc] },
end

-- FIXME simp lemmas for move stuff. should be an attribute somewhere?
-- basically you want `G.adj m.a m.b`, etc. to be a simp lemma all the time
