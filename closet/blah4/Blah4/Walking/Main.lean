import Blah4.Walking.HilbertCurve
...
-- TODO: have this be just an alias
...
theorem invHilbertCurve_injective : Function.Injective invHilbertCurve :=
  Function.Injective.of_comp (f := hilbertCurve) fun x y => by simp
...
theorem walkies {α} (s e : α ↪ EuclideanSpace ℝ (Fin 2)) :
  ...
  . exists fun i => .mk
    ...
    simpa [Function.comp] using Subtype.coe_injective.comp s'.injective
  save
  . exists fun i => .mk
      (.mk
        (fun t => ![(1 - t) * s' i, e' i])
    ...
    simpa [Function.comp] using Subtype.coe_injective.comp e'.injective
  save
  . exists fun i => .mk
      (.mk
        (fun t => ![t * e' i, (1 - t) * e' i])
    ...
    simpa [Function.comp, ← add_mul] using Subtype.coe_injective.comp e'.injective
